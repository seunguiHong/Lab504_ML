...
def run_oos_forecast(X, Y, dates, C, dumploc, ncpus):
    ...
    for step, forecast_idx in enumerate(oos_indices, start=1):
        ...
        if step == 1 or step % log_freq == 0 or step == total_oos:
            ...
            early_abort_threshold = getattr(C, "early_abort_r2_threshold", None)
            early_abort_min_step = int(getattr(C, "early_abort_min_step", 0) or 0)
            early_abort_column = int(getattr(C, "early_abort_column", 0) or 0)
            if early_abort_column < 0:
                early_abort_column += len(r2_now)

            if (
                early_abort_threshold is not None
                and step >= early_abort_min_step
                and 0 <= early_abort_column < len(r2_now)
            ):
                r2_check = float(r2_now[early_abort_column])
                if np.isfinite(r2_check) and r2_check < float(early_abort_threshold):
                    early_aborted = True
                    early_abort_step = step
                    print(
                        "Early abort: "
                        f"R2[{early_abort_column}]={r2_check:.12f} "
                        f"below {float(early_abort_threshold):.12f} "
                        f"at step {step}."
                    )
                    break
    ...
    return {
        ...
        "R2OOS": r2,
        "R2OOS_pval": pval,
        "EarlyAborted": np.array([[early_aborted]], dtype=bool),
        "EarlyAbortStep": np.array([[early_abort_step]], dtype=int),
    }

...
CS_YHAT = nan(nOOS, J);       % ★ 추가: OOS CS 예측값 저장
...
for i = 1:nOOS
    ...
    for j = 1:J
        ...
        err_0(i,j) = yte - yhat0;
        B{i,j}     = Bj;
        CS_YHAT(i,j) = yhat0;                  % ★ 추가: CS 예측값 기록
...
Time_pred     = Time(Time_pred_idx);   % 예측 시점 (yhat이 대응되는 t+H)

% 테이블 구성 (YYYYMM + 예측치)
CS_YHAT_TBL = array2table(CS_YHAT, 'VariableNames', cellstr(Y_cols));
CS_YHAT_TBL.Time_pred = Time_pred;
CS_YHAT_TBL = movevars(CS_YHAT_TBL, 'Time_pred', 'Before', 1);

assignin('base','CS_YHAT_TBL', CS_YHAT_TBL);

% (옵션) OOS 시점의 Time과 함께 테이블로 정리
Time_oos_idx = (idx_burn_end + H) : (idx_burn_end + H + nOOS - 1);
Time_oos     = Time(Time_oos_idx);                      % OOS 예측의 캘린더 타임
CS_TBL = array2table(CS_YHAT, 'VariableNames', cellstr(Y_cols));
CS_TBL.Time = Time_oos;
CS_TBL = movevars(CS_TBL, 'Time', 'Before', 1);
assignin('base','CS_TBL', CS_TBL);

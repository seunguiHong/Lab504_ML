function build_target_and_features_sub(in_csv, out_mat)

    if nargin < 1 || isempty(in_csv),  in_csv = 'dataset.csv'; end
    if nargin < 2 || isempty(out_mat), out_mat = 'target_and_features_sub.mat'; end

    % ------------------------------------------------------------
    % Sample restriction
    % ------------------------------------------------------------
    % Final forecast-origin sample:
    %   2017:12
    %
    % Raw data are kept through:
    %   2018:12
    %
    % Reason:
    %   y.dy(t,:) and y.rx(t,:) are 12-month forward realized targets.
    %   To construct targets for forecast-origin 2017:12, the code needs
    %   yield observations at 2018:12.
    % ------------------------------------------------------------
    sample_end_origin = 201712;
    sample_end_raw    = 201812;
    ...
    Time_num_raw = str2double(string(Time));
    keep_raw = Time_num_raw <= sample_end_raw;
    ...
    %                 - y_t^{(1)}
    % n = 2,...,10
    % Units: percent after multiplying by 100
    % ------------------------------------------------------------
    RX_dec = NaN(T, 9);
    if T > 12
        n_rx = 2:10;
    ...
    % ------------------------------------------------------------
    % Final forecast-origin sample restriction
    % ------------------------------------------------------------
    % Raw data were kept through 2018:12 to construct 12-month targets.
    % Now keep final X/y rows only through forecast-origin 2017:12.
    % ------------------------------------------------------------
    keep_origin = str2double(string(Time)) <= sample_end_origin;

    X = trim_xy_struct(X, keep_origin);
    y = trim_xy_struct(y, keep_origin);

    Time = Time(keep_origin);
    ...
    meta.sample_end_origin = sample_end_origin;
    meta.sample_end_raw    = sample_end_raw;
    meta.sample_note = ...
        'Raw data kept through 201812 to construct 12-month forward targets; final X/y sample trimmed to forecast-origin 201712.';
...
function S = trim_xy_struct(S, keep)
    ...
    for i = 1:numel(fn)
        ...
        if isstruct(S.(name))
            if isfield(S.(name), 'Time')
                S.(name).Time = S.(name).Time(keep, :);
            end

            if isfield(S.(name), 'data')
                S.(name).data = S.(name).data(keep, :);
            end
        end
    end

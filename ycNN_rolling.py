...
BASE_CONFIG = {
    ...
    "window_size": 360,     # rolling window: 360 months = 30 years
    "hyper_freq": 60,
    "nmc": 100,
    "navg": 10,
    "run_tag": "pc123_roll360",
...
def build_model_input_rolling(X, Y, forecast_index, horizon, window_size):
    """
    Build the input expected by NNFuncBib.NNModel using a rolling window.

    Convention:
    - the last row is the forecast row
    - training data end at forecast_index - horizon
    - only the most recent `window_size` training observations are used
    """
    ...
    train_start = max(0, train_end - int(window_size) + 1)
...
def run_oos_forecast(X, Y, dates, cfg, dumploc, ncpus):
    ...
    for j, forecast_index in enumerate(oos_indices, start=1):
        X_model, Y_model = build_model_input_rolling(
            X=X,
            Y=Y,
...
def run_experiment(custom_config=None):
    ...
    print(f"Rolling window size: {cfg['window_size']}")
